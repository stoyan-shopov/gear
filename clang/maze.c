...
#define	X_RESOLUTION	(80 * 2)
#define	Y_RESOLUTION	(24 * 2)
#define TO_RAD(x)	((x) * ((1.0 / 180.0) * M_PI))
#define TO_DEG(X)	((float)(x) * (180.0 / M_PI))
#define ANGLE_STEP	(TO_RAD(VIEW_ANGLE) / (float)(X_RESOLUTION - 1))
...
static float prev_x, prev_y;
	...
	texture_cast_y = (fmod(x, TILE_SIZE) * TEXTURE_RESOLUTION) / (float) TILE_SIZE;

	if (dist_x > dist_y)
	{
		last_texture_byte = texture_y;
		*texture_idx = texture_cast_y;
		printf("cast in y direction, distance is %f, hit at %f, delta %f ", (float) dist_y, x, fabs(x - prev_x));
	}
	else
	{
		last_texture_byte = texture_x;
		*texture_idx = texture_cast_x;
		printf("cast in x direction, distance is %f, hit at %f, delta %f  ", (float) dist_x, y, fabs(y - prev_y));
	}
	prev_x = x;
	prev_y = y;
...
int texture_idx;
	...
	mid_angle = TO_RAD(VIEW_ANGLE) * 0.5;
	for (i = 0; i < X_RESOLUTION; i++)
	{
		...
		dist = cast_ray(camera.pos,
				...
				&texture_idx);
		dist *= cos(mid_angle - i * ANGLE_STEP);
		col_height = 1.0 * TILE_SIZE * Y_RESOLUTION / dist;
		j = round(col_height);
		printf("dist = %f, j == %i\n", dist, j);
		if (j & 1)
			j++;
		tstep = (float) (TEXTURE_RESOLUTION - 1) / (float) (j - 1);
		if (j > Y_RESOLUTION)
			tidx = (float) (j - Y_RESOLUTION) * tstep * 0.5, j = Y_RESOLUTION;
		else
			tidx = .0;
...
int c;
	...
	camera.pos.x = 536.;
	camera.pos.y = 3867.;
	camera.view_angle = M_PI * 0.25;
	camera.view_angle = TO_RAD(-67.5 + 90.0);
	camera.view_angle = TO_RAD(-45. + 90.0);
	while (1)
	{
		...
		side_angle = mid_angle - TO_RAD(90.0);
		sa = sin(mid_angle);
		ca = cos(mid_angle);
