...
gear_engine_request
		...
		|	MEM_READ_KW '(' NUM ',' NUM ')'
		{
			...
			{	
				/* error occurred while reading target memory */
				xprintf(gear_comm_fd, "GEAR_ERR_GENERIC_ERROR\n");
				free(buf);
			}
			else
			{
				/* reading target memory was successful */
				if (nbytes != $5)
					panic("");
				xprintf(gear_comm_fd, "GEAR_ERR_NO_ERROR,");
				i = $5;
		...
		|	REGS_READ_KW '(' NUM ')'
		{
			...
			for (i = 0; i < 16; i++)
				if (regs & (1 << i))
				{
					if (ctx->cc->core_reg_read(ctx, 255, 0, 1 << i, &reg)
							!= GEAR_ERR_NO_ERROR)
						panic("");
			...
			if (regs & (1 << 16))
			{
				if (ctx->cc->core_reg_read(ctx, 255, 0, 1 << 16, &reg)
						!= GEAR_ERR_NO_ERROR)
					panic("");
				xprintf(gear_comm_fd, "0x%08x ", reg);
			}
			if (regs & (1 << 17))
			{
				if (ctx->cc->core_reg_read(ctx, 255, 0, 1 << 17, &reg)
						!= GEAR_ERR_NO_ERROR)
					panic("");
				xprintf(gear_comm_fd, "0x%08x ", reg);
			}
			if (regs & (1 << 18))
			{
				if (ctx->cc->core_reg_read(ctx, 255, 0, 1 << 18, &reg)
						!= GEAR_ERR_NO_ERROR)
					panic("");
				xprintf(gear_comm_fd, "0x%08x ", reg);
			}
		...
		|	reg_write_data NUM
			{
				...
				/* write_reg(...) */
				reg = $2;
				if (ctx->cc->core_reg_write(ctx, 255, 0, 1 << lex_vars.reg_nr, &reg)
						!= GEAR_ERR_NO_ERROR)
					panic("");
				lex_vars.regmask >>= 1, lex_vars.reg_nr ++;
			}
...
struct target_ctl_context * ctx;
	...
	printf("gear engine target core controller for i386-mingw32\n");
	if (argc < 2)
	{
		printf("usage: %s executable_file args\n", * argv);
		exit(1);
	}
	...
	while (1)
	{
		...
		if (ctx->cc->core_open(ctx, argv[1], argc, argv) != GEAR_ERR_NO_ERROR)
			panic("");
