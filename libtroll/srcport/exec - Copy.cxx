	...
	if (!srcfile_name.isNull())
		while (1)
		{
			try
			{
				...
				if (!look_for_files)
					return 0;
                                btn_pressed = QMessageBox::critical(0, "file not found", srcfile_name + " not found", "load file", "do not prompt anymore", "skip");
                                if (btn_pressed == 2)
                                        return 0;
...
QSharedPointer<XTextDocument> exec_display_class::docFromSrcname(const QString &srcname, bool do_disassemble, bool do_fold_disassembly)
{
	...
	QTextCharFormat	disasm_format;
        disasm_format.setForeground(Qt::cyan);
				...
				if (do_disassemble)
				{
					...
					unsigned int memlen, addr;
						...
						addr = srcdata->srcaddr_pairs[t].addr;
                                                        ...
                                                        cursor.insertText(srcinfo->comp_units[srcdata->srcaddr_pairs[t].cuname_idx].name);
						...
						uint64_t pc;
							...
							do
							{
								...
								if (memlen >= 2)
                                                                ...
                                                                {
                                                                        if (openocd_disasm_process->write(QString("%1 %2\n\r\l").arg(pc).arg(opc).toAscii().constData()) == -1)
										errcnt ++;
									Sleep(50);
									openocd_disasm_process->waitForBytesWritten();
									Sleep(50);
									//while (!openocd_disasm_process->canReadLine());
                                                                        QString xxx = QString(openocd_disasm_process->readLine(1024 * 1024));
									qDebug() << xxx;
									qDebug() << "asfasfasfsaf";
                                                                        insn_len = xxx.toUInt();
                                                                        ////insn_len = QString(openocd_disasm_process->readLine(1024 * 1024)).toUInt();
                                                                        insn_text = QString(openocd_disasm_process->readLine(1024));
                                                                }
                                                                else
                                                                {
...
{
default:
	if (phead->type == TROLL_RECORD_SRCINFO)
        {
                struct srcinfo_type_struct * sinfo;
                int i;
			...
			/* source file list already available - do nothing */
                        break;
                sinfo = (struct srcinfo_type_struct *)phead;

                srcdir_list = QStringList();
                qDebug() << "nr_comp_units: " << sinfo->nr_comp_units;
                for (i = 0; i < sinfo->nr_comp_units; i++)
                {
                        srcdir_list.append(QString(sinfo->comp_units[i].compdir));
                }
                srcdir_list.removeDuplicates();
...
void exec_display_class::build_custom_srcdata(struct srcinfo_type_struct * new_srclist)
{
	struct srclist_type_struct * s;
	int i;
	/* process the list of source code files */
	troll_deallocate_record(srcinfo);
	srcinfo = (struct srcinfo_type_struct *) troll_clone(new_srclist);
	s = srcinfo->srclist;
