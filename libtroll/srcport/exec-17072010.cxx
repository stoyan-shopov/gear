...
private:
	...
	QStringList disassembly_list;
	int line_number;
public:
	//~SrcUserData(void) { qDebug() << "user data destroyed"; }
	SrcUserData(void) : QTextBlockUserData(), is_folded(false), has_disassembly(false),line_number(0) {}
	void setFolded(bool folded) { is_folded = folded; }
	bool isFolded(void) { return is_folded; }
	void setHasDisassembly(bool hasDisassembly) { has_disassembly = hasDisassembly; }
	bool hasDisassembly(void) { return has_disassembly; }
	int lineNumber(void) { return line_number; }
	void setLineNumber(int line_number) { this->line_number = line_number; }
	const QStringList & disassList(void) { return disassembly_list; }
	void appendDisassemblyLine(const QString & line) { disassembly_list.append(line); }
...
exec_display_class::exec_display_class(QWidget * parent) :
	...
	margin_area(new QLabel(this)),
        margin_pixmap(new QPixmap(100, 200)),
        src_format(currentCharFormat()),
        disasm_format(currentCharFormat())
{
	setReadOnly(true);
	...
	margin_area->move(contentOffset().toPoint());
	setViewportMargins(4 * fontMetrics().width('x'), 0, 0, 0);
...
void exec_display_class::resizeEvent(QResizeEvent *e)
{
	margin_area->resize(4 * fontMetrics().width('x'), height());
	QPlainTextEdit::resizeEvent(e);
}

void exec_display_class::paintEvent1(QPaintEvent *e)
{
#if 0
int y, dy;
QTextBlock bl = firstVisibleBlock();
SrcUserData * udata;

	p.fillRect(margin_area->rect(), Qt::cyan);
	dy = contentOffset().y();
	if (0) if (bl.isValid()) for (y = 0; y < height(); y += fontMetrics().height())
	{
		if (udata = (SrcUserData *) bl.userData())
			p.drawText(QRect(0, dy, 100, fontMetrics().height()), QString().number(udata->lineNumber()));
		dy += blockBoundingRect(bl).height();
		bl = bl.next();
		if (!bl.isValid())
			break;
	}
	if (margin_pixmap->isNull() || margin_pixmap->height() == 0)
		qDebug() << "pixmap is nul!";
	margin_area->setPixmap(QPixmap(*margin_pixmap));
	margin_area->hide();
...
void exec_display_class::keyPressEvent(QKeyEvent *e)
{
SrcUserData	* udata;
QTextCursor c(textCursor());

	if ((udata = (SrcUserData *) textCursor().block().userData()) && udata->hasDisassembly())
	{
		QStringList	slist = udata->disassList();
		int i;
		if (udata->isFolded())
		{
			...
			{
				c.insertText("\n");
				c.insertText(slist.at(i));
			}
			udata->setFolded(false);
		}
		else
		{
